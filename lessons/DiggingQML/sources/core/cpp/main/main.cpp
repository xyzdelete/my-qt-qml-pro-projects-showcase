...
main(int argc, char* argv[])
{
  ...
  QQmlComponent component(
    &engine,
    ":/DiggingQML/UsingQQmlEngineToLoadQML/sources/qml/files/"
    "UsingQQmlEngineToLoadQML.qml"
  );
  ...
  {
    qDebug() << "Age is : "
             << QQmlProperty::read(child1, "age").toInt();      // Syntax #1
  ...
  if (childRect)
  {
    ...
    qDebug() << "The rect color is :"
             << childRect->property("color").toString();
  ...
  QList<QObject*> deepChildren = rootObject->findChildren<QObject*>(
    "deepChild", Qt::FindChildrenRecursively
  ); // Default Recursive
  if (deepChildren.count() > 0)
  {
    ...
    {
      qDebug() << "The child level is : "
               << object->property("level").toString();
    }
  }
