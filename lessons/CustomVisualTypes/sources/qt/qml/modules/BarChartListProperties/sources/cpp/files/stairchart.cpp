...
)
{
  ...
  painter->drawRect(
    mRect.bottomLeft().x() + startingPoint,
...
void
StairChart::drawScale(QPainter* painter, QRect mRect)
{
  QPen mPen;
  mPen.setWidth(2);
  ...
  painter->drawLine(
    mRect.bottomLeft() + QPoint(0, 0), mRect.bottomLeft() + QPoint(20, 0)
  );
  painter->drawText(mRect.bottomLeft() + QPoint(3, -10), "0");

  painter->drawLine(
    mRect.bottomLeft() + QPoint(0, -100), mRect.bottomLeft() + QPoint(20, -100)
  );
  painter->drawText(mRect.bottomLeft() + QPoint(3, -110), "100");

  painter->drawLine(
    mRect.bottomLeft() + QPoint(0, -200), mRect.bottomLeft() + QPoint(20, -200)
  );
  painter->drawText(mRect.bottomLeft() + QPoint(3, -210), "200");

  painter->drawLine(
    mRect.bottomLeft() + QPoint(0, -300), mRect.bottomLeft() + QPoint(20, -300)
  );
  painter->drawText(mRect.bottomLeft() + QPoint(3, -310), "300");
}

void
StairChart::paint(QPainter* painter)
{
  ...
  for (int i = 0; i < m_stairs.size(); i++)
  {
    ...
    painter->drawRect(
      stairRect.topLeft().x() + 10, stairRect.topLeft().y() + 10, 25, 25
    );
...
QQmlListProperty<Stair>
StairChart::stairs()
{
  return QQmlListProperty<Stair>(
    this, this, &StairChart::appendStair, nullptr, nullptr, nullptr
  );
}

QString
StairChart::title() const
{
  return m_title;
}

QColor
StairChart::bgColor() const
{
  return m_bgColor;
}

void
StairChart::setTitle(QString title)
{
  if (m_title == title)
    return;
...
void
StairChart::setBgColor(QColor bgColor)
{
...
void
StairChart::appendStair(QQmlListProperty<Stair>* list, Stair* stair)
{
