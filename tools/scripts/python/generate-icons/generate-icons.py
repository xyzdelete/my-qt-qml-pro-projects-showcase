...
-------------
    <output-dir>/
      ...
      android/  launcher mipmaps (mdpi-xxxhdpi), one ic_launcher.png each
                + play_store_512.png
      ios/      Assets.xcassets/AppIcon.appiconset/ - a ready-to-drop-in Xcode
                asset catalog. By default: every iPhone/iPad/App Store PNG
                (1x/2x/3x) plus the Contents.json that tells Xcode what each
                file is for. With --ios-single-size: just the 1024px master
                + a matching Contents.json (Xcode 14+'s "Single Size" mode).
...
-----
    ...
    # iOS "Single Size" mode (Xcode 14+): just the 1024px master, instead of
    # all 18 manual iPhone/iPad files. Every other platform is unaffected.
    python generate-icons.py --input-svg-file ./logo.svg --output-dir ./out \\
        --ios-single-size
"""
...
import json  # writes the Xcode asset catalog's Contents.json
...
class Density:
    ...
    launcher: int  # square launcher icon size in px


@dataclass(frozen=True)
class IosIconEntry:
    # One row of an Xcode "AppIcon.appiconset" asset catalog: one <idiom,
    # scale> slot, which Xcode expects as its own PNG file at a specific
    # pixel size (point size x scale factor), listed in Contents.json.
    idiom: str  # "iphone", "ipad", or "ios-marketing" (the App Store slot)
    scale: str  # "1x", "2x", or "3x" — the Retina multiplier Xcode records
    size_pt: float  # point size before scaling (83.5 is real: iPad Pro slot)
    filename: str  # PNG filename Xcode expects inside the .appiconset folder

    @property
    def scale_factor(self) -> int:
        # "2x" -> 2, "3x" -> 3, etc.
        return int(self.scale.rstrip("x"))

    @property
    def pixel_size(self) -> int:
        # The actual PNG must be size_pt * scale_factor pixels square.
        return round(self.size_pt * self.scale_factor)
...
# iOS full icon set (Apple HIG 2026), laid out exactly as Xcode's own asset
# catalog editor would produce it: one row per <idiom, scale> slot, matching
# what ``File > New > iOS > App Icons & Launch Images`` (or a fresh Xcode
# project template) generates in ``Assets.xcassets/AppIcon.appiconset``.
# Filenames follow Xcode's own naming convention (base name + @Nx + optional
# "~ipad" suffix) so the folder is indistinguishable from one Xcode wrote.
#
# (idiom, scale, point size, filename) -> rendered PNG is point-size * scale:
#   iphone  20pt @2x/@3x – Notifications
#   iphone  29pt @2x/@3x – Settings
#   iphone  40pt @2x/@3x – Spotlight
#   iphone  60pt @2x/@3x – Home Screen
#   ipad    20pt @1x/@2x – Notifications
#   ipad    29pt @1x/@2x – Settings
#   ipad    40pt @1x/@2x – Spotlight
#   ipad    76pt @1x/@2x – Home Screen
#   ipad  83.5pt @2x     – Home Screen (iPad Pro)
#   ios-marketing 1024pt @1x – App Store master (required; no transparency)
IOS_APPICONSET: Final[tuple[IosIconEntry, ...]] = (
    IosIconEntry("iphone", "2x", 20, "icon-20@2x.png"),
    IosIconEntry("iphone", "3x", 20, "icon-20@3x.png"),
    IosIconEntry("iphone", "2x", 29, "icon-29@2x.png"),
    IosIconEntry("iphone", "3x", 29, "icon-29@3x.png"),
    IosIconEntry("iphone", "2x", 40, "icon-40@2x.png"),
    IosIconEntry("iphone", "3x", 40, "icon-40@3x.png"),
    IosIconEntry("iphone", "2x", 60, "icon-60@2x.png"),
    IosIconEntry("iphone", "3x", 60, "icon-60@3x.png"),
    IosIconEntry("ipad", "1x", 20, "icon-20@1x~ipad.png"),
    IosIconEntry("ipad", "2x", 20, "icon-20@2x~ipad.png"),
    IosIconEntry("ipad", "1x", 29, "icon-29@1x~ipad.png"),
    IosIconEntry("ipad", "2x", 29, "icon-29@2x~ipad.png"),
    IosIconEntry("ipad", "1x", 40, "icon-40@1x~ipad.png"),
    IosIconEntry("ipad", "2x", 40, "icon-40@2x~ipad.png"),
    IosIconEntry("ipad", "1x", 76, "icon-76@1x~ipad.png"),
    IosIconEntry("ipad", "2x", 76, "icon-76@2x~ipad.png"),
    IosIconEntry("ipad", "2x", 83.5, "icon-83.5@2x~ipad.png"),
    IosIconEntry("ios-marketing", "1x", 1024, "icon-1024.png"),
)
# Used by --ios-single-size mode (Xcode 14+'s "Single Size" app icon):
# just the App Store master, from which Xcode derives every on-device size
# at build time. Kept as its own constants so both modes share one filename
# for the 1024 px master and can't drift apart.
IOS_MARKETING_SIZE: Final[int] = 1024
IOS_SINGLE_SIZE_FILENAME: Final[str] = "icon-1024.png"
# Android density buckets: (mipmap folder name, launcher icon px). This is the
# classic (non-adaptive) launcher icon system: one flat square PNG per bucket,
# named ``ic_launcher.png``, no safe-zone padding or foreground/background
# layers needed.
...
class Context:
    ...
    ios_single_size: bool  # --ios-single-size: iOS "Single Size" mode


def _format_ios_size(size_pt: float) -> str:
    # Xcode writes whole-number points without a decimal ("20x20") but keeps
    # the decimal for the one fractional slot, the iPad Pro's 83.5pt icon
    # ("83.5x83.5"). This reproduces that formatting exactly.
    text: str = str(int(size_pt)) if size_pt == int(size_pt) else str(size_pt)
...
def build_android(ctx: Context) -> None:
    # Android: one flat square ic_launcher.png per screen-density bucket, plus
    # the flat 512 px Play Store listing icon. No adaptive-icon
    # foreground/background layers — just the classic mipmap set.
...
def build_ios(ctx: Context) -> None:
    # iOS: an Xcode asset catalog at ``ios/Assets.xcassets/AppIcon.appiconset/``.
    # Two modes, both producing a valid, Xcode-ready folder + Contents.json:
    #   * default            - the full 18-file manual set (every iPhone/iPad
    #                          slot at 1x/2x/3x, Apple HIG 2026) for maximum
    #                          compatibility and per-size control.
    #   * --ios-single-size - Xcode 14+'s "Single Size" mode: just the
    #                          1024 px master; Xcode derives every on-device
    #                          size from it at build time.
    # Either way, drag ``Assets.xcassets`` (or just ``AppIcon.appiconset``)
    # straight into your project - Xcode recognises it immediately, no
    # import step needed. The 1024 px master has no transparency, as the
    # App Store requires, and is always rendered at exactly 1024x1024.
    appiconset: Path = ctx.out / "ios" / "Assets.xcassets" / "AppIcon.appiconset"
    images: list[dict[str, str]]

    if ctx.ios_single_size:
        # Single Size mode: one file, one Contents.json entry. No "scale"
        # key at all - there's only one image, so scale doesn't apply.
        _png(ctx, IOS_MARKETING_SIZE, appiconset / IOS_SINGLE_SIZE_FILENAME)
        images = [
            {
                "filename": IOS_SINGLE_SIZE_FILENAME,
                "idiom": "universal",
                "platform": "ios",
                "size": _format_ios_size(IOS_MARKETING_SIZE),
            }
        ]
    else:
        # Manual mode: one Contents.json "images" entry per PNG, built
        # alongside the PNGs so the two can never drift out of sync.
        images = []
        for entry in IOS_APPICONSET:
            _png(ctx, entry.pixel_size, appiconset / entry.filename)
            images.append(
                {
                    "idiom": entry.idiom,
                    "scale": entry.scale,
                    "size": _format_ios_size(entry.size_pt),
                    "filename": entry.filename,
                }
            )

    # Xcode's asset-catalog manifest: the image list plus a fixed "info"
    # block recording the tool that wrote it (Xcode itself always writes
    # "xcode"/1 here, so we match that for maximum compatibility).
    contents: dict[str, object] = {
        "images": images,
        "info": {"author": "xcode", "version": 1},
    }
    contents_path: Path = appiconset / "Contents.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "iOS only: write Xcode's 'Single Size' AppIcon.appiconset "
            "(just the 1024px master + a matching Contents.json) instead "
            "of the full 18-file manual set. Every other platform is "
            "unaffected."
        ),
    )
...
def main() -> int:
    ...
    ctx = Context(
        ...
        out=out,
        name=svg.stem,
        ios_single_size=bool(args.ios_single_size),
    )
    try:
