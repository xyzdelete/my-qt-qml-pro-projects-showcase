...
| Thing | Why | Notes |
|-------|-----|-------|
| **Python 3.14.6+** | Runs the script | 3.14.6 or newer recommended |
...
You want **3.14.6 or newer**. If it's missing or too old, install it as below.
...
Look for the highest `Python.Python.3.X.X` entry in the output, then install it
(replace `3.X.X` with the version you saw, e.g. `3.14.6`):

```powershell
winget install -e --id Python.Python.3.X.X
```

To upgrade an existing winget-installed Python later:

```powershell
winget upgrade -e --id Python.Python.3.X.X
```
...
It takes two required arguments — the input SVG and the output directory —
plus one optional iOS-specific flag. The icon name is taken automatically
from the SVG file name.
...
### Optional: `--ios-single-size` (iOS "Single Size" mode)

By default the `ios/` output is the full **manual** icon set: 18 individual
PNGs (every iPhone/iPad size at 1x/2x/3x) plus a `Contents.json` listing each
one explicitly. This works on any Xcode version and gives you per-size
control, but it's more files than modern Xcode strictly needs.

Since Xcode 14, Apple supports **"Single Size"** mode: you provide just the
1024×1024 master, and Xcode generates every on-device size from it
automatically at build time. Pass `--ios-single-size` to get that instead:
...
This only changes the `ios/` output — every other platform (Windows, Linux,
macOS, Android, web) is generated exactly the same either way. With the flag,
`ios/Assets.xcassets/AppIcon.appiconset/` contains just:
...
Both modes produce a **complete, valid `Contents.json`** — Xcode won't
recognise the folder as an app icon set without one, single-size or not.
Either output can be dragged straight into `Assets.xcassets`.

> **Which should you pick?** `--ios-single-size` for any new project on a
> current Xcode version — it's simpler and is what Xcode itself now defaults
> to. Skip the flag (use the full manual set) if your project targets an
> older Xcode version, or if you want to hand-tune specific sizes yourself.
...
│                Contents.json  +  18 PNGs (icon-20@2x.png … icon-1024.png)
│                — or, with --ios-single-size: Contents.json + icon-1024.png only
└── web/       favicon.ico
...
- **android** — icons are placed under `res/mipmap-{density}/` so the directory
  can be used directly as `QT_ANDROID_PACKAGE_SOURCE_DIR`. Just one flat square
  `ic_launcher.png` per density bucket (mdpi 48px, hdpi 72px, xhdpi 96px,
  xxhdpi 144px, xxxhdpi 192px) — no adaptive-icon foreground/background layers
  — plus the 512 px Play Store listing icon.
- **ios** — a complete, ready-to-use **Xcode asset catalog** at
  `Assets.xcassets/AppIcon.appiconset/`. By default: 18 PNGs (Apple HIG 2026,
  all 1x/2x/3x slots for iPhone and iPad — Home Screen, Spotlight, Settings,
  Notifications — plus the 1024 px App Store master) and the `Contents.json`
  manifest that tells Xcode what each file is. With `--ios-single-size`:
  just the 1024 px master and a matching one-entry `Contents.json` (Xcode
  14+'s "Single Size" mode — see [§5](#5-run-the-script) above). Either way
  this is a folder Xcode itself recognises, so you can drag the whole
  `Assets.xcassets` folder or just `AppIcon.appiconset` straight into your
  project — no import step, no re-typing sizes into Xcode's icon editor. The
  1024 px master always has no transparency and is rendered at exactly
  1024×1024 px, since the App Store rejects anything else.
