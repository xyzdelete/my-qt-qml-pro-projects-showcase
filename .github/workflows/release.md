...
| `host-x64-linux-target-arm64-linux` | `ubuntu-latest` | `host-x64-linux-target-arm64-linux` | DEB, TGZ |
...
The Linux arm64 release job uses the x64 `ubuntu-latest` runner with `aarch64-linux-gnu-g++` and the static `host-x64-linux-target-arm64-linux` preset. It cross-compiles the arm64 binary and publishes DEB/TGZ packages. It does not invoke linuxdeploy because this workflow intentionally uses only static vcpkg triplets. The dynamic preset remains available for a future native arm64 AppImage job.
...
Every platform job in this workflow uses a static vcpkg triplet:

| Platform job | Static vcpkg triplet |
| --- | --- |
...
| WebAssembly | `wasm32-emscripten-pthread` |
...
The workflow validates the complete tag with this Bash regular expression:

```bash
^tag-(.+)-release-(v[^[:space:]]+)$
```

The pattern means, from left to right:

| Part | Meaning |
| --- | --- |
| `^` | Start matching at the beginning of the tag. |
| `tag-` | Require the literal prefix `tag-`. |
| `(.+)` | Capture the project name in `BASH_REMATCH[1]`; it must contain at least one character. |
| `-release-` | Require the literal separator `-release-`. |
| `(v[^[:space:]]+)` | Capture the version in `BASH_REMATCH[2]`; it must start with `v` and contain at least one additional non-whitespace character. |
| `$` | End matching at the end of the tag. |

The expression is used inside Bash's `[[ string =~ regex ]]` operator. When it
matches, Bash stores the complete match in `BASH_REMATCH[0]`, the project name
in `BASH_REMATCH[1]`, and the version in `BASH_REMATCH[2]`.

The workflow performs an additional project-name check. A project name cannot
start or end with `-`, and cannot contain `--`. Therefore, this is valid:

```text
tag-Qt6QmlForBeginners-release-v1.2.3
```
...
tag--Qt6QmlForBeginners-release-v1.2.3
tag-Qt6QmlForBeginners--release-v1.2.3
tag-Qt6QmlForBeginners--internal-release-v1.2.3
```

When the workflow is started manually with `workflow_dispatch`, no release tag
is required and the workflow uses `latest` as the version.
