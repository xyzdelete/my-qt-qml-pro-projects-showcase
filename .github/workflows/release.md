...
and entitlements. The workflow currently uses `com.github.xyzdelete.DemoApp`
in the export options, so the uploaded profile must be valid for that
identifier.
...
- `-scheme`: selects the target and build settings to build.
