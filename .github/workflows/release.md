...
| `host-arm64-linux-target-arm64-linux` | `ubuntu-26.04-arm` | `host-arm64-linux-target-arm64-linux` | DEB, TGZ |
...
The Linux arm64 release job uses a native arm64 Ubuntu runner with the static `host-arm64-linux-target-arm64-linux` preset. It builds the arm64 binary natively and publishes DEB/TGZ packages. It does not invoke linuxdeploy because this workflow intentionally uses only static vcpkg triplets.
